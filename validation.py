...
if __name__ == '__main__':
    ...
    # Note: We have followed the same validation preprocessing as in MoCo v2
    ...
    classifier.load_state_dict(torch.load(os.path.join(latest_subdir, 'classifier_79.pt'), map_location=torch.device('cuda')))

    print(f'Classifier Validation Epoch')
    print('------------------------------------------')
    classifier.eval()
    acc1_array = np.array([])
    acc1 = 0
